...
class VarCov:
    ...
    def fit(self, x, w, alpha=0.99, d=1):
        ...
        d: d days VaR and ES, respectively.
        The model uses the linearized loss operator."""
        ...
        VaR = -w.T @ (self.mu_hat * d) + np.sqrt(w.T @ (d * self.cov_hat) @ w) * norm.ppf(alpha)
        ES = -w.T @ (self.mu_hat * d) + np.sqrt(w.T @  (d* self.cov_hat) @ w) * norm.pdf(norm.ppf(alpha))/(1 - alpha)
